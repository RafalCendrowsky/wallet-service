...
class WalletServiceBusinessLogicTests {
    ...
    inner class WalletTests {
        ...
        fun `should create wallet`() {
            val customer = customerService.createCustomer("wallet-test@test.com")
        ...
        fun `should get wallet by id`() {
            val customer = customerService.createCustomer("get-wallet@test.com")
            val created = walletService.createCustomerWallet(customer.id)

            val wallet = walletService.getCustomerWallet(created.id)
        ...
        fun `should throw NotFoundException for unknown wallet`() {
            assertThatThrownBy { walletService.getCustomerWallet(UUID.randomUUID()) }
                .isInstanceOf(NotFoundException::class.java)
        }
        ...
        fun `should return zero balance for new wallet`() {
            ...
            val balance = walletService.getBalance(wallet.id)
        ...
        fun `should refresh a single wallet balance from queue`() {
            ...
            val wallet = walletService.createCustomerWallet(customer.id)
            transferService.createDeposit(wallet.id, BigDecimal("100.00"), UUID.randomUUID().toString())
        ...
        fun `should refresh balances incrementally`() {
            ...
            transferService.createDeposit(wallet.id, BigDecimal("50.00"), UUID.randomUUID().toString())
        ...
        fun `should refresh multiple wallets in one batch`() {
            ...
            val wallet1 = walletService.createCustomerWallet(customer.id)
            val wallet2 = walletService.createCustomerWallet(customer.id)
            transferService.createDeposit(wallet1.id, BigDecimal("100.00"), UUID.randomUUID().toString())
            transferService.createDeposit(wallet2.id, BigDecimal("200.00"), UUID.randomUUID().toString())
    ...
    inner class TransferTests {
        ...
        fun `should deposit money and update balance`() {
            ...
            val transfer = transferService.createDeposit(wallet.id, BigDecimal("100.00"), UUID.randomUUID().toString())
        ...
        fun `should withdraw money and update balance`() {
            ...
            val transfer =
                transferService.createWithdrawal(wallet.id, BigDecimal("40.00"), UUID.randomUUID().toString())
        ...
        fun `should transfer money between wallets`() {
            ...
            val sender = walletService.createCustomerWallet(customer.id)
            val receiver = walletService.createCustomerWallet(customer.id)
            ...
            val senderBalance = walletService.getBalance(sender.id)
            assertThat(senderBalance.balance).isEqualByComparingTo(BigDecimal("50.00"))

            val receiverBalance = walletService.getBalance(receiver.id)
        ...
        fun `should prevent double spending from concurrent requests`() {
            ...
            val sender = walletService.createCustomerWallet(customer.id)
            val receiver1 = walletService.createCustomerWallet(customer.id)
            val receiver2 = walletService.createCustomerWallet(customer.id)
            transferService.createDeposit(sender.id, BigDecimal("100.00"), UUID.randomUUID().toString())
        ...
        fun `should return paginated transaction history`() {
            ...
            val page = transferService.getWalletTransfers(receiver.id, PageRequest.of(0, 3))
    ...
    inner class HoldTests {
        ...
        fun `should place and release hold`() {
            ...
            val hold = holdService.placeHold(wallet.id, BigDecimal("30.00"), LocalDateTime.now().plusDays(1))
            ...
            val releasedBalance = walletService.getBalance(wallet.id)
        ...
        fun `should reject hold when insufficient available balance`() {
            ...
            assertThatThrownBy {
                holdService.placeHold(wallet.id, BigDecimal("30.00"), LocalDateTime.now().plusDays(1))
            }.isInstanceOf(InsufficientFundsException::class.java)
        }
        ...
        fun `should not change balance after failed hold`() {
            ...
            assertThatThrownBy {
                holdService.placeHold(wallet.id, BigDecimal("200.00"), LocalDateTime.now().plusDays(1))
            }.isInstanceOf(InsufficientFundsException::class.java)
        ...
        fun `should handle concurrent holds and transfers competing for same wallet`() {
            ...
            val wallet = walletService.createCustomerWallet(customer.id)
            transferService.createDeposit(wallet.id, BigDecimal("1000.00"), UUID.randomUUID().toString())
            ...
            val tasks = (0 until numThreads).map { i ->
                ...
                Callable {
                    try {
                        barrier.await()
                        if (i % 2 == 0) {
                            holdService.placeHold(wallet.id, BigDecimal("100.00"), LocalDateTime.now().plusDays(1))
                            "HOLD_SUCCESS"
                        } else {
                            transferService.createTransfer(
                                wallet.id,
